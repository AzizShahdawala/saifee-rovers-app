...
export default function Attendance() {
  ...
  return (
    <Box>
      <PageHeader title="Attendance Scanner" subtitle="Live face recognition with instant attendance and manual override." actions={<Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ width: { xs: "100%", sm: "auto" }, "& .MuiButton-root": { width: { xs: "100%", sm: "auto" } } }}><Button variant="outlined" startIcon={<FullscreenOutlined />} onClick={() => scannerSectionRef.current?.requestFullscreen?.()}>Full screen</Button><Button variant="contained" startIcon={<PersonAddOutlined />} onClick={() => setManualOpen(true)}>Manual Attendance</Button></Stack>} />
      <Grid ref={scannerSectionRef} container spacing={2.5} sx={{ bgcolor: "background.default" }}>
        <Grid size={{ xs: 12, lg: 8 }}>
          <Paper sx={{ overflow: "hidden", bgcolor: "#06111f", position: "relative", minHeight: { xs: 320, md: 520 } }}>
            {cameraEnabled ? <Webcam ref={cameraRef} audio={false} screenshotFormat="image/jpeg" videoConstraints={{ facingMode: "user" }} style={{ width: "100%", height: "clamp(320px, 62vh, 520px)", objectFit: "cover", display: "block" }} /> : <Box sx={{ minHeight: { xs: 320, md: 520 }, display: "grid", placeItems: "center", color: "white" }}><Typography>Camera paused</Typography></Box>}
