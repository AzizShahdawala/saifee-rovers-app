...
const Members = () => {
  ...
  const memberChanges = useMemo(() => {
    ...
    const fields = [
      ...
      ["maritalStatus", "Marital status"], ["spouseName", "Spouse name"], ["spouseDateOfBirth", "Spouse date of birth"], ["children", "Children"],
  ...
  const openMember = (member, mode) => {
    ...
    setSaveNotice("");
    setDialogMode(mode);
    const normalized = { ...member, dateOfBirth: member.dateOfBirth?.slice(0, 10) || "", spouseDateOfBirth: member.spouseDateOfBirth?.slice(0, 10) || "", children: (member.children || []).map((child) => ({ ...child, dateOfBirth: child.dateOfBirth?.slice(0, 10) || "" })) };
  ...
  const editFamilyValid = !editMember || editMember.maritalStatus === "UNMARRIED" || (Boolean(editMember.spouseName?.trim()) && Boolean(editMember.spouseDateOfBirth) && (editMember.children || []).every((child) => child.name?.trim() && child.dateOfBirth));
    ...
    <Box>
      ...
      <Dialog open={Boolean(editMember)} onClose={closeMember} fullWidth maxWidth="sm">
        ...
        <DialogContent><Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <TextField disabled={dialogMode === "view"} select label="Marital Status" value={editMember?.maritalStatus || ""} onChange={(event) => setEditMember((current) => ({ ...current, maritalStatus: event.target.value, ...(event.target.value === "UNMARRIED" ? { spouseName: "", spouseDateOfBirth: "", children: [] } : {}) }))} required><MenuItem value="MARRIED">Married</MenuItem><MenuItem value="UNMARRIED">Unmarried</MenuItem></TextField>
