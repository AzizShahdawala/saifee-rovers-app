...
import {
  ...
  RemoveCircleOutlineOutlined as RemoveCircleOutline,
...
import HijriDatePicker from "../../components/member/HijriDatePicker";
import { formatPatrolPeriod, isValidHijriDate } from "../../utils/memberDates";
...
const Members = () => {
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter((member) => {
      const matchesQuery = !query || [member.itsId, member.name, member.email, member.phone, member.hijriDateOfBirth, member.joinedYear, member.patrol, member.instrument, member.profession, member.professionDetails].some(
        (value) =>
          String(value || "")
            .toLowerCase()
            .includes(query),
  ...
  const memberChanges = useMemo(() => {
    ...
    const fields = [
      ["itsId", "ITS ID"], ["name", "Full name"], ["email", "Email"], ["phone", "Phone"], ["dateOfBirth", "Date of birth"], ["hijriDateOfBirth", "Hijri date of birth"], ["joinedYear", "Joined Saifee Rovers"],
    ...
    const display = (key, value) => key === "isPatrolLeader" ? (value ? "Yes" : "No") : key === "profession" ? professionLabel(value) : key === "maritalStatus" ? (value === "MARRIED" ? "Married" : "Unmarried") : key === "children" ? (value?.map((child) => `${child.name} (${child.dateOfBirth})`).join(", ") || "None") : String(value || "Not set");
    return fields.filter(([key]) => JSON.stringify(editMember[key] ?? "") !== JSON.stringify(originalMember[key] ?? "")).map(([key, label]) => ({ key, label, before: display(key, originalMember[key]), after: display(key, editMember[key]) }));
  ...
  const openMember = (member, mode) => {
    ...
    setSaveNotice("");
    setDialogMode(mode);
    const normalized = { ...member, dateOfBirth: member.dateOfBirth?.slice(0, 10) || "", spouseDateOfBirth: member.spouseDateOfBirth?.slice(0, 10) || "", marriageDate: member.marriageDate?.slice(0, 10) || "", children: (member.children || []).map((child) => ({ ...child, dateOfBirth: child.dateOfBirth?.slice(0, 10) || "" })) };
    setEditMember(normalized);
    setOriginalMember(structuredClone(normalized));
  };
  ...
  const editFamilyValid = !editMember || editMember.maritalStatus === "UNMARRIED" || (Boolean(editMember.spouseName?.trim()) && Boolean(editMember.spouseDateOfBirth) && Boolean(editMember.marriageDate) && (editMember.children || []).every((child) => child.name?.trim() && child.dateOfBirth));
  ...
  const columns = [
    ...
    { id: "hijriDateOfBirth", label: "Hijri DOB", sortable: true, minWidth: 130, render: (member) => member.hijriDateOfBirth || "Not set" },
    {
      id: "isPatrolLeader",
      label: "Patrol role",
      minWidth: 130,
    ...
    <Box>
      ...
      <Dialog open={Boolean(editMember)} onClose={closeMember} fullWidth maxWidth="sm">
        ...
        <DialogContent><Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <HijriDatePicker disabled={dialogMode === "view"} value={editMember?.hijriDateOfBirth || ""} onChange={(value) => setEditMember((current) => ({ ...current, hijriDateOfBirth: value }))} required error={dialogMode === "edit" && !isValidHijriDate(editMember?.hijriDateOfBirth)} helperText={dialogMode === "edit" ? "Select the member's Dawoodi Bohra calendar birth date" : "Dawoodi Bohra Misri calendar"} />
          ...
          {editMember?.maritalStatus === "MARRIED" && <TextField disabled={dialogMode === "view"} type="date" label="Marriage Date" value={editMember?.marriageDate || ""} slotProps={{ inputLabel: { shrink: true }, htmlInput: { max: new Date().toISOString().slice(0, 10) } }} onChange={(event) => setEditMember((current) => ({ ...current, marriageDate: event.target.value }))} required />}
          {editMember?.maritalStatus === "MARRIED" && <><TextField disabled={dialogMode === "view"} label="Spouse Name" value={editMember?.spouseName || ""} onChange={(event) => setEditMember((current) => ({ ...current, spouseName: event.target.value }))} required /><TextField disabled={dialogMode === "view"} type="date" label="Spouse Date of Birth" value={editMember?.spouseDateOfBirth || ""} slotProps={{ inputLabel: { shrink: true }, htmlInput: { max: new Date().toISOString().slice(0, 10) } }} onChange={(event) => setEditMember((current) => ({ ...current, spouseDateOfBirth: event.target.value }))} required /><FormControlLabel control={<Checkbox checked={Boolean(editMember?.children?.length)} disabled={dialogMode === "view"} onChange={(event) => setEditMember((current) => ({ ...current, children: event.target.checked ? [{ name: "", dateOfBirth: "" }] : [] }))} />} label="Children" />{editMember.children?.map((child, index) => <Stack key={child._id || index} direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ sm: "flex-start" }}><TextField disabled={dialogMode === "view"} fullWidth label={`Child ${index + 1} Name`} value={child.name || ""} onChange={(event) => setEditMember((current) => ({ ...current, children: current.children.map((item, childIndex) => childIndex === index ? { ...item, name: event.target.value } : item) }))} required /><TextField disabled={dialogMode === "view"} fullWidth type="date" label="Date of Birth" value={child.dateOfBirth || ""} slotProps={{ inputLabel: { shrink: true }, htmlInput: { max: new Date().toISOString().slice(0, 10) } }} onChange={(event) => setEditMember((current) => ({ ...current, children: current.children.map((item, childIndex) => childIndex === index ? { ...item, dateOfBirth: event.target.value } : item) }))} required />{dialogMode === "edit" && <IconButton color="error" onClick={() => setEditMember((current) => ({ ...current, children: current.children.filter((_, childIndex) => childIndex !== index) }))} aria-label={`Remove child ${index + 1}`}><RemoveCircleOutline /></IconButton>}</Stack>)}{dialogMode === "edit" && Boolean(editMember.children?.length) && <Button startIcon={<AddCircleOutline />} onClick={() => setEditMember((current) => ({ ...current, children: [...current.children, { name: "", dateOfBirth: "" }] }))}>Add another child</Button>}</>}
          ...
          {dialogMode === "edit" && originalMember?.patrol !== editMember?.patrol && <Alert severity="info">Saving will close the {originalMember?.patrol} patrol period and start a new {editMember?.patrol} period today.</Alert>}
          <Box sx={{ p: 2, border: "1px solid", borderColor: "divider", borderRadius: 2 }}><Typography fontWeight={800} sx={{ mb: 1 }}>Patrol history</Typography><Stack spacing={1}>{editMember?.patrolHistory?.length ? editMember.patrolHistory.map((entry) => <Box key={entry._id || `${entry.patrol}-${entry.fromDate}`} sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", gap: .5 }}><Typography fontWeight={700}>{entry.patrol}</Typography><Typography variant="body2" color="text.secondary">{formatPatrolPeriod(entry)}</Typography></Box>) : <Typography variant="body2" color="text.secondary">History will begin from the member&apos;s joined year.</Typography>}</Stack></Box>
