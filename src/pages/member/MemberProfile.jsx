...
export default function MemberProfile() {
  ...
  useEffect(() => {
    getMemberProfile().then(({ data }) => setMember(data.member)).catch((requestError) => setError(errorMessage(requestError, "Could not load profile")));
  }, []);
  ...
  const selectPhoto = async (event) => {
    ...
    if (!["image/jpeg", "image/png"].includes(photo.type) || photo.size > 5 * 1024 * 1024) {
      setError("Choose a JPG or PNG image no larger than 5 MB");
      return;
    }
    setPhotoLoading(true); setError(""); setNotice("");
    try {
      const { data } = await updateMemberProfilePhoto(photo);
  ...
  const submitPassword = async () => {
    ...
    try {
      const { data } = await changeMemberPassword({ currentPassword: passwords.currentPassword, newPassword: passwords.newPassword });
  ...
  const fields = [["ITS ID", member.itsId], ["Full name", member.name], ["Email", member.email], ["Phone", member.phone], ["Date of birth", new Date(member.dateOfBirth).toLocaleDateString("en-IN", { dateStyle: "long" })], ["Profession", professionLabel(member.profession)], ["Profession details", member.professionDetails || "Not applicable"], ["Marital status", member.maritalStatus === "MARRIED" ? "Married" : "Unmarried"], ["Spouse", member.spouseName || "Not applicable"], ["Spouse date of birth", member.spouseDateOfBirth ? new Date(member.spouseDateOfBirth).toLocaleDateString("en-IN", { dateStyle: "long" }) : "Not applicable"], ["Children", member.children?.length ? member.children.map((child) => `${child.name} (${new Date(child.dateOfBirth).toLocaleDateString("en-IN")})`).join(", ") : "None"], ["Patrol", member.patrol], ["Instrument", member.instrument || "Not assigned"], ["Member status", member.status], ["Joined", new Date(member.createdAt).toLocaleDateString("en-IN", { dateStyle: "long" })]];
  ...
  return <Stack spacing={3}>
    ...
    <Paper sx={{ p: { xs: 2.5, sm: 4 }, maxWidth: 820 }}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={3} alignItems={{ xs: "stretch", sm: "center" }}>
        <Box sx={{ position: "relative", width: 104, alignSelf: { xs: "center", sm: "auto" } }}><Avatar src={member.profileImage} sx={{ width: 104, height: 104, fontSize: 38 }}>{member.name?.[0]}</Avatar>{photoLoading && <Box sx={{ position: "absolute", inset: 0, borderRadius: "50%", bgcolor: "rgba(255,255,255,.75)", display: "grid", placeItems: "center" }}><CircularProgress size={30} /></Box>}</Box>
